...
class As2PartnerEnvironment(BaseModel):
    """
    ...
    require_signature: Optional[StrictBool] = Field(default=True, description="When `true` (the secure default), ModernEDI rejects non-MDN inbound messages for this environment unless the sender signature is present and successfully verified with the configured certificate. This policy belongs to the AS2 connection/environment and therefore applies to every attached partner on a shared or VAN connection. Set `false` only for a documented connection-wide exception. Omission on create resolves to `true`; resolved connection responses always include the field.", alias="requireSignature")
    require_encryption: Optional[StrictBool] = Field(default=True, description="When `true` (the secure default), ModernEDI rejects non-MDN inbound messages for this environment unless their encrypted content was successfully decrypted. This policy belongs to the AS2 connection/environment and therefore applies to every attached partner on a shared or VAN connection. Set `false` only for a documented connection-wide exception. Omission on create resolves to `true`; resolved connection responses always include the field.", alias="requireEncryption")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of As2PartnerEnvironment from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of As2PartnerEnvironment from a dict"""
        if obj is None:
            return None
