...
class ImplementationAcknowledgmentDetail(BaseModel):
    """
    ...
    """ # noqa: E501
    message_id: Optional[StrictStr] = Field(description="AS2 Message-Id of the stored 999, or `null` when only partial acknowledgment data was retained.", alias="messageId", json_schema_extra={"examples": ["implementation-ack-850-api"]})
    sent: Optional[StrictBool] = Field(description="Delivery/persistence completion flag from the retained 999 record. True means its acknowledgment lifecycle completed, false means it did not, and `null` means the state is unavailable. This is not sender direction; use the transaction direction and timeline event type to determine whether ModernEDI sent or received the 999.")
    ...
    mdn: Optional[TransactionMdn] = Field(description="AS2 receipt assurance state for the 999, or `null` when no MDN applies or was recorded.")
    outcome: ImplementationAcknowledgmentOutcome
    evaluation_scope: Optional[StrictStr] = Field(description="Whether this transaction was evaluated against its exact AK2/IK5 result, a unique AK1/AK9 group, or the conservative whole-999 aggregate.", alias="evaluationScope")
    transaction_outcome: Optional[ImplementationAcknowledgmentTransactionSetOutcome] = Field(description="Exact AK2/IK5 outcome attributable to this transaction, or `null`.", alias="transactionOutcome")
    evaluated_status: Optional[ImplementationAcknowledgmentStatus] = Field(description="Status selected for this transaction at `evaluationScope`, or `null` when no evaluation was available.", alias="evaluatedStatus")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["messageId", "sent", "timestamp", "partnerConfigurationSha256", "mdn", "outcome", "evaluationScope", "transactionOutcome", "evaluatedStatus"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ImplementationAcknowledgmentDetail from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ImplementationAcknowledgmentDetail from a dict"""
        if obj is None:
            return None
