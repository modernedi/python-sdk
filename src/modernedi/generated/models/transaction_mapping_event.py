...
class TransactionMappingEvent(BaseModel):
    """
    ...
    type: StrictStr = Field(description="Identifies whether the attempt failed, succeeded, or resolved an earlier failure.")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mapping.failed', 'mapping.succeeded', 'mapping.recovered']):
            raise ValueError("must be one of enum values ('mapping.failed', 'mapping.succeeded', 'mapping.recovered')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TransactionMappingEvent from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TransactionMappingEvent from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({key: value for key, value in {
            ...
            "details": TransactionMappingAttempt.from_dict(obj["details"]) if obj.get("details") is not None else None
        }.items() if key in obj})
