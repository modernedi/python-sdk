...
from modernedi.generated.models.configuration_desired_file_content import ConfigurationDesiredFileContent
from typing import Optional, Set
from typing_extensions import Self
from pydantic_core import to_jsonable_python

class ConfigurationDesiredFile(BaseModel):
    """
    ...
    """ # noqa: E501
    path: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Safe relative POSIX-style path outside `_state`. The sole MANIFEST path is exactly `modernedi.json`.")
    role: StrictStr = Field(description="Desired-file purpose. STATE is deliberately excluded because observed server identity is not portable desired configuration.")
    format: StrictStr = Field(description="MANIFEST and RESOURCE files use JSON; SOURCE and CERTIFICATE files use TEXT.")
    media_type: StrictStr = Field(description="MANIFEST and RESOURCE files use `application/json`; SOURCE and CERTIFICATE files use exact UTF-8 text.", alias="mediaType")
    ...
    def role_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MANIFEST', 'RESOURCE', 'SOURCE', 'CERTIFICATE']):
            raise ValueError("must be one of enum values ('MANIFEST', 'RESOURCE', 'SOURCE', 'CERTIFICATE')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ConfigurationDesiredFile from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ConfigurationDesiredFile from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({key: value for key, value in {
            ...
            "content": ConfigurationDesiredFileContent.from_dict(obj["content"]) if obj.get("content") is not None else None
        }.items() if key in obj})
        return _obj
