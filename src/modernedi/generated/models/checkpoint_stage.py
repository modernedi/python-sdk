...
class CheckpointStage(BaseModel):
    ...
    kind: StrictStr = Field(description="Emit a declared global checkpoint after the preceding stages complete. In v1, the containing step must have occurrence max 1 so the emitting occurrence is unambiguous.")
    ...
    __properties: ClassVar[List[str]] = ["id", "kind", "checkpoint"]
    ...
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['checkpoint']):
            raise ValueError("must be one of enum values ('checkpoint')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CheckpointStage from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CheckpointStage from a dict"""
        if obj is None:
            return None
