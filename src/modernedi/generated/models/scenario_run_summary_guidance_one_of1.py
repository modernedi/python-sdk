...
class ScenarioRunSummaryGuidanceOneOf1(BaseModel):
    """
    ScenarioRunSummaryGuidanceOneOf1
    """ # noqa: E501
    next_action: StrictStr = Field(alias="nextAction")
    step_id: Optional[StrictStr] = Field(alias="stepId")
    occurrence: Optional[StrictStr]
    __properties: ClassVar[List[str]] = ["nextAction", "stepId", "occurrence"]

    @field_validator('next_action')
    def next_action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['attach_or_refresh']):
            raise ValueError("must be one of enum values ('attach_or_refresh')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ScenarioRunSummaryGuidanceOneOf1 from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ScenarioRunSummaryGuidanceOneOf1 from a dict"""
        if obj is None:
            return None
