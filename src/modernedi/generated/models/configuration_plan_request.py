...
class ConfigurationPlanRequest(BaseModel):
    """
    ...
    files: Annotated[List[ConfigurationPlanFile], Field(min_length=1, max_length=5000)] = Field(description="Desired files plus, optionally, the single exported advisory state file. Paths must be safe, case-insensitively unique relative POSIX paths, and each declared hash must match the file's logical bytes. Only `_state/snapshot.json` may use the `_state` prefix.")
    __properties: ClassVar[List[str]] = ["refreshScenarioBindings", "files"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ConfigurationPlanRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ConfigurationPlanRequest from a dict"""
        if obj is None:
            return None
