...
class TransactionMdnEventDetails(BaseModel):
    """
    Safe normalized AS2 receipt diagnostics used by MDN timeline events.
    """ # noqa: E501
    message_id: Optional[StrictStr] = Field(description="Message-ID of the MDN itself.", alias="messageId")
    disposition: Optional[StrictStr] = Field(description="Raw AS2 Disposition value.")
    status: StrictStr = Field(description="Normalized receipt-assurance state that determines the event type.")
    ...
    mic_matched: Optional[StrictBool] = Field(description="Whether the returned MIC matched the sent canonical content.", alias="micMatched")
    ...
    validation_error: Optional[StrictStr] = Field(description="Receipt validation or disposition problem, or `null`.", alias="validationError")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["messageId", "disposition", "status", "asynchronous", "micMatched", "expectedBy", "validationError"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TransactionMdnEventDetails from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TransactionMdnEventDetails from a dict"""
        if obj is None:
            return None
