...
class MappedOutputAckResponse(BaseModel):
    """
    Confirmation that one leased mapped output moved into acknowledged-by-your-integration custody.
    """ # noqa: E501
    success: StrictBool = Field(description="Always `true`; stale handles or unknown outputs use the documented error response instead.")
    environment: TransactionEnvironmentValue
    acknowledgment: MappedOutputAcknowledgment
    id: StrictStr = Field(description="Mapped output id that was marked received.")
    message_id: StrictStr = Field(description="Original inbound AS2 message id for the mapped output.", alias="messageId")
    ...
    __properties: ClassVar[List[str]] = ["success", "environment", "acknowledgment", "id", "messageId", "mappedOutputKey", "ackedAt"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MappedOutputAckResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MappedOutputAckResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({key: value for key, value in {
            ...
            "acknowledgment": MappedOutputAcknowledgment.from_dict(obj["acknowledgment"]) if obj.get("acknowledgment") is not None else None,
            "id": obj.get("id"),
