...
class TransactionMappedOutputEvent(BaseModel):
    """
    ...
    type: StrictStr = Field(description="Identifies the exact creation or managed-delivery transition represented by this event.")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mapped_output.created', 'mapped_output.available', 'mapped_output.delivered', 'mapped_output.redelivered', 'mapped_output.attention_required', 'mapped_output.acknowledged']):
            raise ValueError("must be one of enum values ('mapped_output.created', 'mapped_output.available', 'mapped_output.delivered', 'mapped_output.redelivered', 'mapped_output.attention_required', 'mapped_output.acknowledged')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TransactionMappedOutputEvent from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TransactionMappedOutputEvent from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({key: value for key, value in {
            ...
            "details": StoredTransactionMappedOutput.from_dict(obj["details"]) if obj.get("details") is not None else None
        }.items() if key in obj})
