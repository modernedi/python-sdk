...
PREVIEWOUTBOUNDX12REQUEST_ANY_OF_SCHEMAS = ["Dict[str, object]", "List[object]", "bool", "float", "str"]

class PreviewOutboundX12Request(BaseModel):
    """
    PreviewOutboundX12Request
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PreviewOutboundX12Request.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PreviewOutboundX12Request with anyOf schemas: Dict[str, object], List[object], bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PreviewOutboundX12Request with anyOf schemas: Dict[str, object], List[object], bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
