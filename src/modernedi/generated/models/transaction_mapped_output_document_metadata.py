...
class TransactionMappedOutputDocumentMetadata(BaseModel):
    """
    ...
    content_type: Optional[StrictStr] = Field(description="Media type of the corresponding mapped-output document body, or `null` for historical artifacts without recorded content type.", alias="contentType", json_schema_extra={"examples": ["application/json"]})
    created_at: Optional[str] = Field(description="UTC RFC 3339 instant with exactly nine fractional digits and a trailing `Z`, or `null`.", alias="createdAt", json_schema_extra={"examples": ["2026-07-15T18:30:00Z"]})
    business_key: Optional[TransactionBusinessKey] = Field(description="Map-derived reconciliation identifier, or `null` when the map did not produce one.", alias="businessKey")
    mapping: TransactionMappedOutputMapping
    extra_fields: Dict[str, StrictStr] = Field(description="Runtime routing metadata captured with the output; this is not the map's selectable delivered context.", alias="extraFields")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TransactionMappedOutputDocumentMetadata from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TransactionMappedOutputDocumentMetadata from a dict"""
        if obj is None:
            return None
